...
client.on("interactionCreate", async (interaction) => {
  const endedEmbed = new MessageEmbed()
    .setTitle(`<:icons_no:1018202282657058866> | Voting has been terminated`)
    .setColor("RED")
    .setFooter({
      iconURL: client.user.avatarURL({ dynamic: true }),
      text: "Treo Bot ∙ DEVANSH#4075",
    })
      ...
      if (value === "visible") {
        ...
        if (interaction.user.id != data.OwnerID)
          return interaction.reply({
            embeds: [
              new MessageEmbed()
                .setTitle(`<:icons_no:1018202282657058866> | You are not the creator of the vote`)
                .setFooter({
                  iconURL: client.user.avatarURL({ dynamic: true }),
                  text: "Treo Bot ∙ DEVANSH#4075",
                })
        ...
        interaction.reply({content: `${ad}`,
          embeds: [
            new MessageEmbed()
              .setTitle(`<:icons_yes:1018200822837952606> | Voting is set to ${textPublic}`)
              .setFooter({
                iconURL: client.user.avatarURL({ dynamic: true }),
                text: "Treo Bot ∙ Devansh#4075",
              })
  ...
  if (interaction.isButton) {
    if (interaction.customId.startsWith("Option")) {
      ...
      interaction.reply({
        embeds: [
          new MessageEmbed()
            .setTitle("<:icons_yes:1018200822837952606> | voting is complete")
            .setDescription(
